...
st.title("📊 Резултати по блокове (Максимална цена)")
...
if uploaded_file is not None:
    try:
        ...
        # 1. Вземаме точно 12-те най-високи цени в целия файл
        top_12 = df.sort_values(by='Цена (EUR/MWh)', ascending=False).head(12).sort_index()

        # 2. Групиране в блокове
        blocks = []
        if not top_12.empty:
        ...
        # 3. Показване
        st.subheader("⏳ Най-скъпи периоди за работа:")
